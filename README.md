...
| 4.1 | Absolute start-of-range **L2 liquidity snapshot** (`tick_snapshot.py`) + RPC usage logger (`usage.py` → `usage.csv`) | 📋 planned |
| 4.2 | Use the initial liquidity in Stage 3 plots (absolute depth curve) | 📋 planned |
| 4.3 | Interactive, shareable **order book over time** (time slider, Plotly HTML) | 📋 planned |

Tests: `python tests.py` → **27 passing** (stdlib `unittest`, one `test_stage{1,2,3}.py` per stage;
...
plot.py                           # Stage 3 — render PNGs (matplotlib, Agg backend)
tests.py + test_stage{1,2,3}.py   # stdlib-unittest runner + per-stage tests
requirements.txt                  # web3, matplotlib (pinned)
PLAN.md                           # the working plan (incl. Stage 4)
...
separate → L3; aggregate per tick → L2. **Stage 4** (planned, see `PLAN.md`) adds the absolute L2
baseline (4.1), folds it into the Stage-3 plots (4.2), and builds the interactive book-over-time
figure with a time slider (4.3).
